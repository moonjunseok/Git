...
b
# 키에는 아이템의 값, 값에는 해당 아이템의 개수가 들어간 딕셔너리를 생성한다.
# 가장 빈도 수가 높은 요소는 most_common()을 이용한다.
b.most_common(2) # b에서 빈도수가 높은 요소 2개를 호출해라

# 타입선언
a = list()
type(a) # 이처럼 이름으로 리스트를 선언하고 타입을 확인하는 방법이 있다. 하지만 기호로도 가능하다.
type([])
type(())
type({})
type({1})